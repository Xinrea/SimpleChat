...
{
    account = ui->accountEdit->text().toInt();
    ui->label->setText("");
    if(account >= 65535){
...
void MainWindow::loginNow(bool control)
{
    if(control)
    {
        loginModule.login(account,password);
        mainW->initConfig(loginModule.account,loginModule.session,loginModule.port);
    }
    else loginModule.logout();
}
